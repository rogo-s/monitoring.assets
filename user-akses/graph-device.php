        ...
        <div class="main-panel">
            <div class="content">
                <div class="panel-header bg-primary-gradient">
                    <div class="page-inner py-5">
                        <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
                            <div>
                                <h2 class="text-white pb-2 fw-bold">Dashboard Listing Device</h2>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="row mt--2">
                    ...
                    <?php
                    // Ganti query untuk chart domain
                    $domainQuery = "SELECT domain.domain_name, COUNT(*) as count FROM device_assets_computer dac 
                    LEFT JOIN tbl_domain domain ON dac.domain_id = domain.domain_id GROUP BY domain.domain_name";
                    $queryDomain = $db->query($domainQuery);

                    // Inisialisasi array yang akan menyimpan hasil perhitungan
                    $domainCount = array();

                    // Mengisi array dengan hasil perhitungan dari database
                    while ($row = $queryDomain->fetch_assoc()) {
                        $domainName = $row['domain_name'];
                        $count = (int)$row['count'];
                        $domainCount[$domainName] = $count;
                    }

                    // Mengonversi array menjadi format JSON
                    $dataDomain = json_encode($domainCount);
                    ?>
                    ...
                    <div class="col-md-6">
                        <div class="card full-height">
                            ...
                            <div class="card-body">
                                <div class="chart-container">
                                    <canvas id="barChartBrand" style="width: 50%; height: 50%"></canvas>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <?php
                    // Ganti query untuk chart brand
                    $brandQuery = "SELECT brand.brand_name, COUNT(*) as count FROM device_assets_computer dac 
                    LEFT JOIN tbl_brand brand ON dac.brand_id = brand.brand_id GROUP BY brand.brand_name";
                    $queryBrand = $db->query($brandQuery);
                    ...
                    while ($row = $queryBrand->fetch_assoc()) {
                        ...
                        $count = (int)$row['count'];
                        $brandCount[$brandName] = $count;
                    }

                    // Mengonversi array menjadi format JSON
                    $dataBrand = json_encode($brandCount);
                    ?>
                </div>
            </div>
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    ...
                                    <div class="row mt-3">
                                        <div class="col-md-12">
                                            <form id="search-form-device">
                                                <div class="input-group mb-1">
                                                    <input type="text" class="form-control" placeholder="Cari berdasarkan Serial Number, Nama Merek, dll..." name="keyword" id="search-keyword-device">
                                ...
                                <div class="card-head-row card-tools-still-right">
                                    <div class="table-responsive table-wrapper">
                                        <table id="add-row-device" class="display table table-striped table-hover">
                                            <thead>
                                                <tr>
                                                    <th></th>
                                                    <th>Serial Number</th>
                                            ...
                                            <tbody>
                                                ...
                                                $sqlDevice = "SELECT 
                                                    dc.device_id, dc.serial_number, b.brand_name, dc.laptop_type, dm.domain_name,
                                                    tw.windows_name, twb.windows_bit, ta.antivirus_name, dc.antivirus_version, ts.status_device
                                                    FROM device_assets_computer dc
                                                    LEFT JOIN tbl_brand b ON dc.brand_id = b.brand_id
                                                    LEFT JOIN tbl_domain dm ON dc.domain_id = dm.domain_id
                                                    LEFT JOIN tbl_windows tw ON dc.windows_id = tw.windows_id
                                                    LEFT JOIN tbl_wbit twb ON dc.wbit_id = twb.wbit_id
                                                    LEFT JOIN tbl_antivirus ta ON dc.antivirus_id = ta.antivirus_id
                                                    LEFT JOIN tbl_status ts ON dc.status_id = ts.status_id
                                                    LIMIT $start, $perPage";

                                                $queryDevice = $db->query($sqlDevice);

                                                while ($row = mysqli_fetch_array($queryDevice)) {
                                                    echo "<tr>";
            ...
            var chartDomain = <?php echo $dataDomain; ?>;
            var chartBrand = <?php echo $dataBrand; ?>;

            // Membuat skema warna yang bervariasi
            var colorSchemeDomain = generateColorScheme(Object.keys(chartDomain).length);
            var colorSchemeBrand = generateColorScheme(Object.keys(chartBrand).length);

            // Membuat chart pie menggunakan Chart.js untuk domain
            var pieChart = document.getElementById('pieChart').getContext('2d');
            var myPieChart = new Chart(pieChart, {
                type: 'pie',
                data: {
                    labels: Object.keys(chartDomain),
                    datasets: [{
                        data: Object.values(chartDomain),
                        backgroundColor: colorSchemeDomain,
                        borderWidth: 0,
                    }],
                },
                options: {
            ...
            // Membuat chart bar menggunakan Chart.js untuk brand
            var barChartBrand = document.getElementById('barChartBrand').getContext('2d');
            var myBarChartBrand = new Chart(barChartBrand, {
                type: 'bar',
                data: {
                    labels: Object.keys(chartBrand),
                    datasets: [{
                        label: 'Brands',
                        data: Object.values(chartBrand),
                        backgroundColor: colorSchemeBrand,
                        borderWidth: 0,
                    }],
                },
                options: {
                    ...
                    maintainAspectRatio: false,
                    legend: {
                        display: false, // Menghilangkan legend agar tidak tampil ganda
                    },
                    scales: {
                        x: {
                            stacked: true,
                ...
                // Fungsi pencarian untuk tabel device
                $("#search-form-device").on("submit", function(event) {
                    event.preventDefault();
                    const keyword = $("#search-keyword-device").val().toLowerCase();
                    $("#add-row-device tbody tr").each(function() {
